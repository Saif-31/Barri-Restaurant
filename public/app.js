...
function addTranscript(speaker, message, isSystem = false) {
  ...
  if (isSystem) {
    ...
    item.innerHTML = `
      ${speaker === 'user' ? '👤 You' : '🍽️ Il-Barri AI'}
      ${message}
    `;
  }
...
function handleServerMessage(message) {
  ...
  switch (message.type) {
    ...
    case 'response.function_call_arguments.done':
      ...
      // Handle restaurant-specific function calls
      if (message.name === 'create_reservation') {
        try {
          ...
          addTranscript('system', `
            ...
            ${args.special_requests ? '📝 Special Requests: ' + args.special_requests : ''}
          `, true);
        } catch (e) {
          console.error('Failed to parse function arguments:', e);
        }
      } else if (message.name === 'create_order') {
        try {
          const args = JSON.parse(message.arguments);
          const itemsList = args.items.map(item => `  • ${item.quantity}x ${item.item_name}${item.modifications ? ' (' + item.modifications + ')' : ''}`).join('\n');
          addTranscript('system', `
            ✅ Order Confirmed!
            👤 Name: ${args.customer_name}
            📦 Type: ${args.order_type}
            🍽️ Items:\n${itemsList}
            ${args.delivery_address ? '📍 Delivery to: ' + args.delivery_address : ''}
            📞 Phone: ${args.customer_phone}
          `, true);
        } catch (e) {
          console.error('Failed to parse function arguments:', e);
        }
      } else if (message.name === 'check_table_availability') {
        try {
          const args = JSON.parse(message.arguments);
          addTranscript('system', `
      ...
      } else if (message.name === 'get_menu_items') {
        try {
          const args = JSON.parse(message.arguments);
          addTranscript('system', `
            📖 Fetching menu...
            ${args.category ? '🍽️ Category: ' + args.category : ''}
            ${args.dietary_filter ? '🥗 Filter: ' + args.dietary_filter : ''}
          `, true);
        } catch (e) {
          console.error('Failed to parse function arguments:', e);
        }
